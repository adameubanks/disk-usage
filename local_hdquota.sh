...
function_gpfs_usage() {
    ...
    if [[ -z $USED_GB || -z $SIZE_GB || -z $FILES ]]; then
        echo "Error: Unable to retrieve GPFS usage data."
        return
    fi
    ...
    USE=$(echo | awk -v u=$USED_GB -v s=$SIZE_GB '{if (s > 0) printf "%d", u * 100 / s; else print "N/A"}')
}
